...
view.View = class {
    ...
    error(err, name, screen) {
        ...
        window.alert = () => {
            // open(location, '_self').close(); //关浏览器
            window.close() //关窗口
        }
        ...
        const knowns = [
            ...
            },
            {
                ...
                url: 'https://github.com/lutzroeder/netron/issues/539'
            },
            {
                name: 'Error loading Keras model.',
                message: /^Invalid argument identifier/,
            ...
            },
            {
                ...
                url: 'https://github.com/lutzroeder/netron/issues/561'
            },
            {
                name: 'RangeError',
        ...
        return this._timeout(2).then(() => {
            ...
            return this._modelFactoryService.open(context).then((model) => {
                ...
                return this._timeout(20).then(() => {
                    ...
                    let model_str = JSON.stringify(graph)
                    let model_json = JSON.parse(model_str)

                    // 经测试 win下字段为identifier，原因未知
                    try {
                        var filename = context._identifier.toString() + '.json'
                    } catch (err) {
                        var filename = context.identifier.toString() + '.json'
                    }


                    this.save_file(model_str, filename)
        ...
        try {
            ...
            if (!graph) {
                ...
                for (const node of nodes) {
                    ...
                    if (nodeName) {
                        g.setNode(nodeId, {
                            label: element.format(graphElement),
                            id: 'node-' + nodeName,
                            class: 'graph-node'
                        });
                    } else {
    ...
    _openSignature(context) {
        ...
        const list = [
            ...
            {name: 'keras-yolo2 configuation', value: /^{\s*"model"\s*:\s*{\s*"architecture"/},
            {
                ...
                value: /^[\s\S]*name:\s*[\s\S]*platform:\s*[\s\S]*input\s*\[[\s\S]*\][\s\S]*output\s*\[[\s\S]*\]/
            }
        ];
